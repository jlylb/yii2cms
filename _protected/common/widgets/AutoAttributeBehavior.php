 ...
 * @since 2.0
 */
class AutoAttributeBehavior extends AttributeBehavior
{
    /**
   ...
   // public $value;
        ...
        if (!empty($this->attributes[$event->name])) {
            ...
            foreach ($attributes as $attribute=>$value) {
                ...
                if (is_string($attribute)) {
                    $this->owner->$attribute = $this->getValue($event,$value);
                }
            }
        }
    }
     ...
     */
    protected function getValue($event,$vaule='')
    {
        return $vaule instanceof Closure ? call_user_func($vaule, $event) : $vaule;
    }
}
